;=================================================================================
PRO Vol_CreateVolume, bin=bi, xrange=xr, yrange=yr, zrange=zr, stackdir=stackdir, stacktype=stacktype, vgi=vgi
    schichtcomb=0
    ...
    IF schichtcomb EQ 0 THEN BEGIN
      ...
      xmin=FIX(recon_abs_query('xmin', logname=recopfad+'/reconlog.txt', /noinput))
      xmax=FIX(recon_abs_query('xmax', logname=recopfad+'/reconlog.txt', /noinput))

      xrecsize=xmax-xmin+1
      yrecsize=xmax-xmin+1

      zmin=FIX(recon_abs_query('ymin', logname=recopfad+'/reconlog.txt', /noinput))
      zmax=FIX(recon_abs_query('ymax', logname=recopfad+'/reconlog.txt', /noinput))
   ...
   ;pixelsize=FLOAT(recon_abs_query('eff_pix_binned'))*bi
   pixelsize=recon_abs_query('eff_pix_binned')*bi
